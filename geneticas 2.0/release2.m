...
for n = base:length(population)
    ...
    v = X2;
    nfi=length(v);
    p=config(15); %p é o numero de pontos usados para descrever o perfil, fixo
    ...
    fprintf(fid,'PACC\n');
    fprintf(fid,"%s \n\n", sname);
    for i=1:(length(config)-21)/2
        fprintf(fid,"Alfa %1.1f \n", config(20+2*i));
    end
    
    if config(6+config(21))~=0
        fprintf(fid,"Alfa %1.1f \n", config(6+config(21)));
    end
...
for n = base:length(population)
    ...
    if isfile(sname) && f.bytes ~= 0
        ...
        CLCD=0;
        alfa = zeros ((length(config)-21)/2,1);
        CL = zeros ((length(config)-21)/2,1);
        CD = zeros ((length(config)-21)/2,1);
        CLCD_aux = zeros ((length(config)-21)/2,1);
        
        for j=1:(length(config)-21)/2
            % Separate Cp data
            alfa(j)  = dataBuffer{1}(j);
            CL(j) = dataBuffer{2}(j);
            CD(j) = dataBuffer{3}(j);
            CLCD_aux(j) = CL(j)/CD(j);
            CLCD = CLCD + CLCD_aux(j)*config(1,2*j+21);
        end
        
        if config(5+config(21)) ~= 0 && (chord_max < config(5+config(21)))
            CLCD=CLCD*(abs((chord_max/config(5+config(21)))^2-0.1));
        end
...
function saveData(population, config)
    ...
    nameOfFile = join([day "_" month "_" year "__" hour "," minutes '.csv'],"");
    %fid = fopen(nameOfFile,"w");
    %close(fid);
    writetable(struct2table(population),nameOfFile)
end
...
for i=1:config(3+config(21))
    ...
    population(n).n = i;
    population(n).name = "perfil-" + int2str(gen) + "." + int2str(i) + ".txt";
end
end
